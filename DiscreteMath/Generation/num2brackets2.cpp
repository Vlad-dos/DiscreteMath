...
#define all(a) (a).begin(), (a).end()
	...
	int main() {
		...
		for (int i = 0; i < 2 * n; i++) {
			...
			if (brackets[2 * n - (i + 1)][balance + 1] * (1 << ((2 * n - i - 2 - balance) / 2)) > k) {
				ans += "(";
				st.push(')');
				balance++;
				continue;
			...
			if (!st.empty() && st.top() == ')') {
				if (brackets[2 * n - (i + 1)][balance - 1] * (1 << ((2 * n - i - balance) / 2)) > k) {
					ans += ")";
					st.pop();
					balance--;
					continue;
...
#if defined(num2brackets2) && !defined(checker)
int main() {
#ifdef LOCAL
